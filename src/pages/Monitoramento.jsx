...
import { 
  ...
  Chip, TextField, Divider, Modal, Button
} from '@mui/material';
...
export default function Monitoramento() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      } catch (err) { console.error("Erro ao buscar telemetria:", err); }
    }, 3000);
  ...
  const handleDelete = async (id, nome) => {
    if (!podeEditar) {
      showToast({ type: "error", title: "Sem permissão", message: "Você não tem permissão para inativar idosos." });
      return;
    }

    const ok = await confirmar(
      `Inativar ${nome}?`,
      "Esta ação irá remover o idoso do monitoramento ativo."
    );
    if (!ok) return;

    try {
      await api.delete(`/idosos/${id}`);
      showToast({ type: "success", title: "Idoso inativado", message: `${nome} foi removido com sucesso.` });
      fetchIdosos();
    } catch (error) {
      const msg = error.response?.data?.message || "Erro interno.";
  ...
  const handleEdit = (id) => {
    if (!podeEditar) {
      showToast({ type: "error", title: "Sem permissão", message: "Você não tem permissão para editar informações dos idosos." });
      return;
    }
  ...
  const salvarEdicao = async () => {
    ...
    try {
      ...
      showToast({ type: "success", title: "Dados atualizados!", message: `Informações de ${idosoEditando.nome} salvas.` });
      fecharModal();
      fetchIdosos();
    } catch (err) {
      const msgErro = err.response?.data?.detail || err.response?.data?.message || "Erro interno.";
      showToast({ type: "error", title: "Erro ao atualizar", message: msgErro });
    }
  };
  ...
  return (
    <Box component="section" sx={{ p: { xs: 2, sm: 4 }, bgcolor: '#f4f6f8', minHeight: '100vh' }}>
      ...
      {/* ConfirmDialog precisa estar no JSX para funcionar */}
      <ConfirmDialog />
      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' },
        ...
        <TextField label="Pesquisar por nome ou serial..." variant="outlined" size="small"
          ...
          return (
            <Grid key={idoso.id} item xs={12} sm={6} md={4} lg={3} component="li">
              <Card sx={{ borderRadius: 4, boxShadow: '0 10px 30px rgba(0,0,0,0.05)',
                border: '1px solid #e0e0e0', overflow: 'visible', transition: 'all 0.3s ease',
                ...
                <CardContent sx={{ p: 3 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                    ...
                    <Chip label={idoso.ativo ? "ATIVO" : "INATIVO"} color={idoso.ativo ? "success" : "default"}
                      size="small" sx={{ fontWeight: 'bold', fontSize: '0.65rem' }} />
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <DirectionsRunIcon sx={{ mr: 0.5, fontSize: 20, color: corMovimento(dados.acelerometro) }} />
                      <Typography variant="body2" fontWeight="bold" sx={{ color: corMovimento(dados.acelerometro) }}>
      ...
      <Modal open={openModal} onClose={fecharModal} aria-label="Editar dados do idoso">
        <Box component="section" sx={{
          ...
          width: { xs: '90%', sm: 450 }, bgcolor: 'white', borderRadius: 4, p: 4, boxShadow: 24,
        }}>
