  ...
  // Inicializo os estados do formulário com valores padrão
  ...
  // Crio o estado para gerenciar o feedback visual (carregamento, sucesso e erro)
  const [status, setStatus] = useState({ loading: false, error: null, success: false });
  const tipos = ['GESTOR', 'CUIDADOR', 'ENFERMEIRO', 'FAMILIAR'];

  // Atualizo o estado do formulário dinamicamente conforme o usuário digita
  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData(prev => ({ ...prev, [name]: value }));
  };

  // Submeto os dados para a API ao enviar o formulário
  const handleSubmit = async (e) => {
    e.preventDefault();
    setStatus({ loading: true, error: null, success: false });

    // Verifico e converto o ID do asilo para garantir que é um número válido
    ...
    // Monto o payload final anexando o ID do asilo
    const payload = { ...formData, asiloId: idAsilo };

    try {
      // Realizo a requisição POST para o backend
      await axios.post('http://localhost:8080/usuarios', payload);
      
      // Limpo os dados do formulário e indico sucesso na operação
      setStatus({ loading: false, error: null, success: true });
      setFormData({ nome: '', email: '', senha: '', cpf: '', tipoUsuario: 'CUIDADOR' });
      
      // Chamo o callback opcional para notificar componentes pai (como o Dashboard)
      if (onSucesso) onSucesso();
      
    } catch (err) {
      // Capturo o erro disparado pelo GlobalExceptionHandler (buscando a propriedade 'detail' do ProblemDetail)
